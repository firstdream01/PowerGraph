...
#ifndef GRAPHLAB_DISTRIBUTED_SWR_INGRESS_HPP
#define GRAPHLAB_DISTRIBUTED_SWR_INGRESS_HPP
...
namespace graphlab {
  ...
  public:
    distributed_swr_ingress(distributed_control& dc, graph_type& graph, bool usehash = false, bool userecent = false) :
      base_type(dc, graph),
    ...
    ~distributed_swr_ingress() { }

    /** Add an edge to the ingress object using swr greedy assignment. */
    void add_edge(vertex_id_type source, vertex_id_type target,
      ...
      const procid_t owning_proc = 
        base_type::edge_decision.edge_to_proc_swr(source, target, dht[source], dht[target], degree_dht[source], degree_dht[target], proc_num_edges, usehash, userecent);
